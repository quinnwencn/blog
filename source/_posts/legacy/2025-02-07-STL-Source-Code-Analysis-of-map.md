...
map与set一样，也是使用STL的红黑树为底层实现，使用适配器适配出的一个容器，区别在于map的值类型是一个pair：
```Cpp
...
// typedefs:
  ...
  typedef pair<const _Key, _Tp> value_type;    // 红黑树的元素类型(键值/实值)，注意，键值是一个const类型
  typedef _Compare              key_compare;   // 键值比较函数
    
  // 下面定义一个functor,其作用就是调用“元素比较函数”， 实际上存在的必要性不大，可以和set一样作为一个模板参数由用户提供
  class value_compare
...
public:
  ...
  // 注意，map insert和set一样，使用底层RB-tree的insert_unique()而非insert_equal()
  ...
  // accessors:
  // 以下所有的map操作行为，RG-tree都已提供，适配器就是这么玩的，用底层的接口适配
...
[source issue](https://github.com/quinnwencn/blog/issues/103)
